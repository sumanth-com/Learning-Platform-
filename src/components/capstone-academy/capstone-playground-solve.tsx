...
import { MonacoMarkdownReference } from "@/components/capstone-academy/workspace/monaco-markdown-reference";
import { MonacoJsonReference } from "@/components/capstone-academy/workspace/monaco-json-reference";
...
type CapstonePlaygroundSolveProps = {
  moduleSlug: string;
  topicSlug: string;
  topicTitle: string;
  moduleTitle: string;
  challenge: CapstoneChallenge;
};
...
export function CapstonePlaygroundSolve({
  moduleSlug,
  topicSlug,
  topicTitle,
  moduleTitle,
  challenge,
}: CapstonePlaygroundSolveProps) {
  ...
  const topic = useMemo(() => getCapstoneAcademyTopic(topicSlug), [topicSlug]);
  const learnAbout = cleanText(
    challenge.scenario ||
      topic?.summary ||
      "Scope, architecture, and delivery planning."
  );
  ...
  const capstoneRefs = topic?.cheatSheet ?? [];
  ...
  const siblings = useMemo(
    () => listCapstoneAcademyChallenges(topicSlug),
    [topicSlug]
  );
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-[#0d0d0d]">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 overflow-hidden lg:grid-cols-2">
        <div className="capstone-challenge-scroll min-h-0 space-y-5 overflow-y-auto border-r border-zinc-800 px-4 py-5 sm:px-5">
          ...
          {capstoneRefs.length > 0 ? (
            <div>
              ...
              >
                ...
                {refsOpen ? "Hide capstone reference" : "Show capstone reference"}
              </Button>
              {refsOpen ? (
                <Section title="Capstone reference">
                  <ul className="space-y-3">
                    {capstoneRefs.map((ref) => (
