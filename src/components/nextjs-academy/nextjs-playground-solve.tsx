...
import { MonacoJsxReference } from "@/components/react-academy/workspace/monaco-jsx-reference";
import { CURRICULUM_ROUTES } from "@/features/curriculum/lib/curriculum-routes";
import {
  listNextjsAcademyChallenges,
  type NextjsChallenge,
...
type NextjsPlaygroundSolveProps = {
  moduleSlug: string;
  topicSlug: string;
  topicTitle: string;
  moduleTitle: string;
  challenge: NextjsChallenge;
};
...
export function NextjsPlaygroundSolve({
  moduleSlug,
  topicSlug,
  topicTitle,
  moduleTitle,
  challenge,
}: NextjsPlaygroundSolveProps) {
  ...
  const [codeTab, setCodeTab] = useState<"html" | "jsx">("jsx");
  ...
  const topic = useMemo(() => getNextjsAcademyTopic(topicSlug), [topicSlug]);
  const learnAbout = cleanText(
    challenge.scenario ||
      topic?.summary ||
      "Next.js is the full-stack React framework for production apps - App Router, Server Components, and routing conventions."
  );

  const htmlCode = challenge.referenceHtml || challenge.starterHtml;
  const jsxCode = challenge.referenceJsx || challenge.starterJsx;
  const nextRefs = topic?.cheatSheet ?? [];
  ...
  const siblings = useMemo(
    () => listNextjsAcademyChallenges(topicSlug),
    [topicSlug]
  );
  ...
  const activeCode = codeTab === "html" ? htmlCode : jsxCode;
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-[#0d0d0d]">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 overflow-hidden lg:grid-cols-2">
        <div className="nextjs-challenge-scroll min-h-0 space-y-5 overflow-y-auto border-r border-zinc-800 px-4 py-5 sm:px-5">
          ...
          {nextRefs.length > 0 ? (
            <div>
              ...
              >
                ...
                {refsOpen ? "Hide Next.js reference" : "Show Next.js reference"}
              </Button>
              {refsOpen ? (
                <Section title="Next.js reference">
        ...
        <div className="flex min-h-0 flex-col overflow-hidden bg-[#0a0a0b]">
          ...
          <div className="relative min-h-0 flex-1 overflow-visible">
            {codeTab === "html" ? (
              ...
              <MonacoJsxReference value={jsxCode} />
            )}
          </div>
