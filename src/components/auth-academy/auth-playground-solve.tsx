...
export function AuthPlaygroundSolve({
  moduleSlug,
  topicSlug,
  topicTitle,
  moduleTitle,
  challenge,
}: AuthPlaygroundSolveProps) {
  ...
  const [codeTab, setCodeTab] = useState<"http" | "js">("js");
  ...
  const topic = useMemo(() => getAuthAcademyTopic(topicSlug), [topicSlug]);
  const learnAbout = cleanText(
    challenge.scenario ||
      topic?.summary ||
      "Sessions, tokens, and secure defaults for web apps."
  );

  const httpCode = challenge.referenceHttp || challenge.starterHttp;
  const jsCode = challenge.referenceJs || challenge.starterJs;
  const authRefs = topic?.cheatSheet ?? [];
  ...
  const siblings = useMemo(
    () => listAuthAcademyChallenges(topicSlug),
    [topicSlug]
  );
  ...
  const activeCode = codeTab === "http" ? httpCode : jsCode;
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-[#0d0d0d]">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 overflow-hidden lg:grid-cols-2">
        <div className="auth-challenge-scroll min-h-0 space-y-5 overflow-y-auto border-r border-zinc-800 px-4 py-5 sm:px-5">
          ...
          {authRefs.length > 0 ? (
            <div>
              ...
              >
                ...
                {refsOpen ? "Hide Auth reference" : "Show Auth reference"}
              </Button>
              {refsOpen ? (
                <Section title="Auth reference">
        ...
        <div className="flex min-h-0 flex-col overflow-hidden bg-[#0a0a0b]">
          ...
          <div className="relative min-h-0 flex-1 overflow-visible">
            {codeTab === "http" ? (
              <MonacoHttpReference value={httpCode} />
            ) : (
