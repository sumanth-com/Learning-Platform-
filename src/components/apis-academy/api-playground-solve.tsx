...
type ApiPlaygroundSolveProps = {
  moduleSlug: string;
  topicSlug: string;
  topicTitle: string;
  moduleTitle: string;
  challenge: ApisChallenge;
};
...
export function ApiPlaygroundSolve({
  moduleSlug,
  topicSlug,
  topicTitle,
  moduleTitle,
  challenge,
}: ApiPlaygroundSolveProps) {
  ...
  const siblings = useMemo(
    () => listApisAcademyChallenges(topicSlug),
    [topicSlug]
  );
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-[#0d0d0d]">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 overflow-hidden lg:grid-cols-2">
        <div className="apis-challenge-scroll min-h-0 space-y-5 overflow-y-auto border-r border-zinc-800 px-4 py-5 sm:px-5">
          ...
          {apiRefs.length > 0 ? (
            <div>
              ...
              >
                ...
                {refsOpen ? "Hide API reference" : "Show API reference"}
              </Button>
              {refsOpen ? (
