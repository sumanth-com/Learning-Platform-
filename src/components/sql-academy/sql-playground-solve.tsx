...
export function SqlPlaygroundSolve({
  moduleSlug,
  topicSlug,
  topicTitle,
  moduleTitle,
  challenge,
}: SqlPlaygroundSolveProps) {
  ...
  const [codeTab, setCodeTab] = useState<"schema" | "sql">("sql");
  ...
  const topic = useMemo(() => getSqlAcademyTopic(topicSlug), [topicSlug]);
  const learnAbout = cleanText(
    challenge.scenario ||
      topic?.summary ||
      "SQL, schemas, and query fundamentals for relational databases."
  );
  ...
  const sqlRefs = topic?.cheatSheet ?? [];
  ...
  const siblings = useMemo(
    () => listSqlAcademyChallenges(topicSlug),
    [topicSlug]
  );
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-[#0d0d0d]">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 overflow-hidden lg:grid-cols-2">
        <div className="sql-challenge-scroll min-h-0 space-y-5 overflow-y-auto border-r border-zinc-800 px-4 py-5 sm:px-5">
          ...
          {sqlRefs.length > 0 ? (
            <div>
              ...
              >
                ...
                {refsOpen ? "Hide SQL reference" : "Show SQL reference"}
              </Button>
              {refsOpen ? (
                <Section title="SQL reference">
