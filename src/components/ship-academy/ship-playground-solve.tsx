...
import { MonacoMarkdownReference } from "@/components/ship-academy/workspace/monaco-markdown-reference";
import { MonacoShellReference } from "@/components/ship-academy/workspace/monaco-shell-reference";
...
type ShipPlaygroundSolveProps = {
  moduleSlug: string;
  topicSlug: string;
  topicTitle: string;
  moduleTitle: string;
  challenge: ShipChallenge;
};
...
export function ShipPlaygroundSolve({
  moduleSlug,
  topicSlug,
  topicTitle,
  moduleTitle,
  challenge,
}: ShipPlaygroundSolveProps) {
  ...
  const [codeTab, setCodeTab] = useState<"markdown" | "shell">("markdown");
  ...
  const topic = useMemo(() => getShipAcademyTopic(topicSlug), [topicSlug]);
  const learnAbout = cleanText(
    challenge.scenario ||
      topic?.summary ||
      "Polish, launch, and present your work."
  );
  ...
  const shipRefs = topic?.cheatSheet ?? [];
  ...
  const siblings = useMemo(
    () => listShipAcademyChallenges(topicSlug),
    [topicSlug]
  );
  ...
  const activeCode = codeTab === "markdown" ? markdownCode : shellCode;
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-[#0d0d0d]">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 overflow-hidden lg:grid-cols-2">
        <div className="ship-challenge-scroll min-h-0 space-y-5 overflow-y-auto border-r border-zinc-800 px-4 py-5 sm:px-5">
          ...
          {shipRefs.length > 0 ? (
            <div>
              ...
              >
                ...
                {refsOpen ? "Hide ship reference" : "Show ship reference"}
              </Button>
              {refsOpen ? (
                <Section title="Ship reference">
