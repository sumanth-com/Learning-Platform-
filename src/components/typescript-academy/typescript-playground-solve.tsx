...
import { MonacoTsReference } from "@/components/typescript-academy/workspace/monaco-ts-reference";
import { CURRICULUM_ROUTES } from "@/features/curriculum/lib/curriculum-routes";
import {
  listTypescriptAcademyChallenges,
  type TypescriptChallenge,
...
type TypescriptPlaygroundSolveProps = {
  moduleSlug: string;
  topicSlug: string;
  topicTitle: string;
  moduleTitle: string;
...
export function TypescriptPlaygroundSolve({
  moduleSlug,
  topicSlug,
  topicTitle,
  moduleTitle,
  challenge,
}: TypescriptPlaygroundSolveProps) {
  ...
  const [codeTab, setCodeTab] = useState<"html" | "ts">("ts");
  ...
  const topic = useMemo(() => getTypescriptAcademyTopic(topicSlug), [topicSlug]);
  const learnAbout = cleanText(
    challenge.scenario ||
      topic?.summary ||
      "TypeScript adds static types to JavaScript so editors and the compiler can catch mistakes before runtime."
  );

  const htmlCode = challenge.referenceHtml || challenge.starterHtml;
  const tsCode = challenge.referenceTs || challenge.starterTs;
  const tsRefs = topic?.cheatSheet ?? [];
  ...
  const siblings = useMemo(
    () => listTypescriptAcademyChallenges(topicSlug),
    [topicSlug]
  );
  ...
  const activeCode = codeTab === "html" ? htmlCode : tsCode;
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-[#0d0d0d]">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 overflow-hidden lg:grid-cols-2">
        <div className="ts-challenge-scroll min-h-0 space-y-5 overflow-y-auto border-r border-zinc-800 px-4 py-5 sm:px-5">
          ...
          {tsRefs.length > 0 ? (
            <div>
              ...
              >
                ...
                {refsOpen ? "Hide TS reference" : "Show TS reference"}
              </Button>
              {refsOpen ? (
                <Section title="TS reference">
        ...
        <div className="flex min-h-0 flex-col overflow-hidden bg-[#0a0a0b]">
          ...
          <div className="relative min-h-0 flex-1 overflow-visible">
            {codeTab === "html" ? (
              ...
              <MonacoTsReference value={tsCode} />
            )}
          </div>
