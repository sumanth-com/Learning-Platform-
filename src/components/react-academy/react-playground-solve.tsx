...
type ReactPlaygroundSolveProps = {
  moduleSlug: string;
  topicSlug: string;
  topicTitle: string;
  moduleTitle: string;
...
export function ReactPlaygroundSolve({
  moduleSlug,
  topicSlug,
  topicTitle,
  moduleTitle,
  challenge,
}: ReactPlaygroundSolveProps) {
  ...
  const topic = useMemo(() => getReactAcademyTopic(topicSlug), [topicSlug]);
  const learnAbout = cleanText(
    challenge.scenario ||
      topic?.summary ||
      "React builds UIs from components. Props pass data in; state and effects drive interactive behavior."
  );

  const htmlCode = challenge.referenceHtml || challenge.starterHtml;
  const jsxCode = challenge.referenceJsx || challenge.starterJsx;
  const reactRefs = topic?.cheatSheet ?? [];
  ...
  const siblings = useMemo(
    () => listReactAcademyChallenges(topicSlug),
    [topicSlug]
  );
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-[#0d0d0d]">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 overflow-hidden lg:grid-cols-2">
        <div className="react-challenge-scroll min-h-0 space-y-5 overflow-y-auto border-r border-zinc-800 px-4 py-5 sm:px-5">
          ...
          {reactRefs.length > 0 ? (
            <div>
              ...
              >
                ...
                {refsOpen ? "Hide React reference" : "Show React reference"}
              </Button>
              {refsOpen ? (
                <Section title="React reference">
