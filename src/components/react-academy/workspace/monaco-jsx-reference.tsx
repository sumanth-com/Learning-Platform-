...
type MonacoJsxReferenceProps = {
  value: string;
  className?: string;
};

/**
 * Read-only Monaco JSX/TSX viewer — scroll + select/copy only.
 */
function MonacoJsxReferenceInner({
  value,
  className,
}: MonacoJsxReferenceProps) {
    ...
    <div className={cn("relative h-full min-h-0 overflow-visible bg-[#1e1e1e]", className)}>
      <Editor
        ...
        options={{
          ...
          domReadOnly: true,
          fixedOverflowWidgets: true,
          ariaLabel: "React JSX code reference",
        }}
      />
    </div>
  );
...
export const MonacoJsxReference = memo(MonacoJsxReferenceInner);
