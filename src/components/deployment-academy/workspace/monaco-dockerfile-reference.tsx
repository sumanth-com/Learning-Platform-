...
type MonacoDockerfileReferenceProps = {
  value: string;
  className?: string;
};

function MonacoDockerfileReferenceInner({
  value,
  className,
}: MonacoDockerfileReferenceProps) {
    ...
    <div className={cn("relative h-full min-h-0 overflow-visible bg-[#1e1e1e]", className)}>
      <Editor
        ...
        options={{
          ...
          domReadOnly: true,
          fixedOverflowWidgets: true,
          ariaLabel: "Dockerfile reference",
        }}
      />
    </div>
  );
...
export const MonacoDockerfileReference = memo(MonacoDockerfileReferenceInner);
