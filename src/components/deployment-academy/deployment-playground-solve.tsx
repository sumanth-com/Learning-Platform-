...
import { MonacoDockerfileReference } from "@/components/deployment-academy/workspace/monaco-dockerfile-reference";
import { MonacoShellReference } from "@/components/deployment-academy/workspace/monaco-shell-reference";
...
type DeploymentPlaygroundSolveProps = {
  moduleSlug: string;
  topicSlug: string;
  topicTitle: string;
  moduleTitle: string;
...
export function DeploymentPlaygroundSolve({
  moduleSlug,
  topicSlug,
  topicTitle,
  moduleTitle,
  challenge,
}: DeploymentPlaygroundSolveProps) {
  ...
  const [codeTab, setCodeTab] = useState<"dockerfile" | "shell">("shell");
  ...
  const topic = useMemo(() => getDeploymentAcademyTopic(topicSlug), [topicSlug]);
  const learnAbout = cleanText(
    challenge.scenario ||
      topic?.summary ||
      "Environments, hosting, containers, and release basics."
  );

  const dockerfileCode = challenge.referenceDockerfile || challenge.starterDockerfile;
  const shellCode = challenge.referenceShell || challenge.starterShell;
  const deployRefs = topic?.cheatSheet ?? [];
  ...
  const siblings = useMemo(
    () => listDeploymentAcademyChallenges(topicSlug),
    [topicSlug]
  );
  ...
  const activeCode = codeTab === "dockerfile" ? dockerfileCode : shellCode;
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-[#0d0d0d]">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 overflow-hidden lg:grid-cols-2">
        <div className="deploy-challenge-scroll min-h-0 space-y-5 overflow-y-auto border-r border-zinc-800 px-4 py-5 sm:px-5">
          ...
          {deployRefs.length > 0 ? (
            <div>
              ...
              >
                ...
                {refsOpen ? "Hide deploy reference" : "Show deploy reference"}
              </Button>
              {refsOpen ? (
                <Section title="Deploy reference">
        ...
        <div className="flex min-h-0 flex-col overflow-hidden bg-[#0a0a0b]">
          ...
          <div className="relative min-h-0 flex-1 overflow-visible">
            {codeTab === "dockerfile" ? (
              <MonacoDockerfileReference value={dockerfileCode} />
            ) : (
              <MonacoShellReference value={shellCode} />
            )}
          </div>
