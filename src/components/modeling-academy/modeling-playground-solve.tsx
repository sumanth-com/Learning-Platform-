...
import { MonacoHttpReference } from "@/components/apis-academy/workspace/monaco-http-reference";
import { MonacoSqlReference } from "@/components/sql-academy/workspace/monaco-sql-reference";
import { CURRICULUM_ROUTES } from "@/features/curriculum/lib/curriculum-routes";
import {
  listModelingAcademyChallenges,
  type ModelingChallenge,
...
type ModelingPlaygroundSolveProps = {
  moduleSlug: string;
  topicSlug: string;
  topicTitle: string;
  moduleTitle: string;
...
export function ModelingPlaygroundSolve({
  moduleSlug,
  topicSlug,
  topicTitle,
  moduleTitle,
  challenge,
}: ModelingPlaygroundSolveProps) {
  ...
  const [codeTab, setCodeTab] = useState<"schema" | "sql">("schema");
  ...
  const topic = useMemo(() => getModelingAcademyTopic(topicSlug), [topicSlug]);
  const learnAbout = cleanText(
    challenge.scenario ||
      topic?.summary ||
      "Design schemas that scale with your product."
  );

  const schemaCode = challenge.referenceSchema || challenge.starterSchema;
  const sqlCode = challenge.referenceSql || challenge.starterSql;
  const modelingRefs = topic?.cheatSheet ?? [];
  ...
  const siblings = useMemo(
    () => listModelingAcademyChallenges(topicSlug),
    [topicSlug]
  );
  ...
  const activeCode = codeTab === "schema" ? schemaCode : sqlCode;
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-[#0d0d0d]">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 overflow-hidden lg:grid-cols-2">
        <div className="modeling-challenge-scroll min-h-0 space-y-5 overflow-y-auto border-r border-zinc-800 px-4 py-5 sm:px-5">
          ...
          {modelingRefs.length > 0 ? (
            <div>
              ...
              >
                ...
                {refsOpen ? "Hide Modeling reference" : "Show Modeling reference"}
              </Button>
              {refsOpen ? (
                <Section title="Modeling reference">
                  <ul className="space-y-3">
                    {modelingRefs.map((ref) => (
        ...
        <div className="flex min-h-0 flex-col overflow-hidden bg-[#0a0a0b]">
          ...
          <div className="relative min-h-0 flex-1 overflow-visible">
            {codeTab === "schema" ? (
              <MonacoHttpReference value={schemaCode} />
            ) : (
              <MonacoSqlReference value={sqlCode} />
            )}
          </div>
