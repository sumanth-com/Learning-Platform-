...
import { MonacoPromptReference } from "@/components/ai-features-academy/workspace/monaco-prompt-reference";
import { MonacoJsReference } from "@/components/ai-features-academy/workspace/monaco-js-reference";
import { CURRICULUM_ROUTES } from "@/features/curriculum/lib/curriculum-routes";
import {
  listAiFeaturesAcademyChallenges,
  type AiFeaturesChallenge,
...
type AiFeaturesPlaygroundSolveProps = {
  moduleSlug: string;
  topicSlug: string;
  topicTitle: string;
  moduleTitle: string;
  challenge: AiFeaturesChallenge;
};
...
export function AiFeaturesPlaygroundSolve({
  moduleSlug,
  topicSlug,
  topicTitle,
  moduleTitle,
  challenge,
}: AiFeaturesPlaygroundSolveProps) {
  ...
  const [codeTab, setCodeTab] = useState<"prompt" | "js">("prompt");
  ...
  const topic = useMemo(() => getAiFeaturesAcademyTopic(topicSlug), [topicSlug]);
  const learnAbout = cleanText(
    challenge.scenario ||
      topic?.summary ||
      "Prompting, tools, and product integration patterns."
  );

  const promptCode = challenge.referencePrompt || challenge.starterPrompt;
  const jsCode = challenge.referenceJs || challenge.starterJs;
  const aiRefs = topic?.cheatSheet ?? [];
  ...
  const siblings = useMemo(
    () => listAiFeaturesAcademyChallenges(topicSlug),
    [topicSlug]
  );
  ...
  const activeCode = codeTab === "prompt" ? promptCode : jsCode;
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-[#0d0d0d]">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 overflow-hidden lg:grid-cols-2">
        <div className="ai-features-challenge-scroll min-h-0 space-y-5 overflow-y-auto border-r border-zinc-800 px-4 py-5 sm:px-5">
          ...
          {aiRefs.length > 0 ? (
            <div>
              ...
              >
                ...
                {refsOpen ? "Hide AI reference" : "Show AI reference"}
              </Button>
              {refsOpen ? (
                <Section title="AI reference">
        ...
        <div className="flex min-h-0 flex-col overflow-hidden bg-[#0a0a0b]">
          ...
          <div className="relative min-h-0 flex-1 overflow-visible">
            {codeTab === "prompt" ? (
              <MonacoPromptReference value={promptCode} />
            ) : (
