...
import { MonacoJsReference } from "@/components/interview-academy/workspace/monaco-js-reference";
import { MonacoMarkdownReference } from "@/components/interview-academy/workspace/monaco-markdown-reference";
...
export function InterviewPlaygroundSolve({
  ...
  const [codeTab, setCodeTab] = useState<"js" | "markdown">("js");
  ...
  const topic = useMemo(() => getInterviewAcademyTopic(topicSlug), [topicSlug]);
  const learnAbout = cleanText(
    challenge.scenario ||
      topic?.summary ||
      "Coding patterns and communication under pressure."
  );
  ...
  const interviewRefs = topic?.cheatSheet ?? [];
  ...
  const activeCode = codeTab === "js" ? jsCode : markdownCode;
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-[#0d0d0d]">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 overflow-hidden lg:grid-cols-2">
        <div className="interview-challenge-scroll min-h-0 space-y-5 overflow-y-auto border-r border-zinc-800 px-4 py-5 sm:px-5">
          ...
          {interviewRefs.length > 0 ? (
            <div>
              ...
              >
                ...
                {refsOpen ? "Hide interview reference" : "Show interview reference"}
              </Button>
              {refsOpen ? (
                <Section title="Interview reference">
                  <ul className="space-y-3">
                    {interviewRefs.map((ref) => (
