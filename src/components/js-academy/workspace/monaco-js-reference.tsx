...
type MonacoJsReferenceProps = {
  value: string;
  className?: string;
};

/**
 * Read-only Monaco JavaScript viewer — scroll + select/copy only.
 */
function MonacoJsReferenceInner({
  value,
  className,
}: MonacoJsReferenceProps) {
    ...
    <div className={cn("relative h-full min-h-0 overflow-visible bg-[#1e1e1e]", className)}>
      <Editor
        ...
        options={{
          ...
          domReadOnly: true,
          fixedOverflowWidgets: true,
          ariaLabel: "JavaScript code reference",
        }}
      />
    </div>
  );
...
export const MonacoJsReference = memo(MonacoJsReferenceInner);
