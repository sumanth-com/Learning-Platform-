...
import { MonacoPromptReference } from "@/components/llm-academy/workspace/monaco-prompt-reference";
import { MonacoJsReference } from "@/components/llm-academy/workspace/monaco-js-reference";
...
type LlmPlaygroundSolveProps = {
  moduleSlug: string;
  topicSlug: string;
  topicTitle: string;
  moduleTitle: string;
  challenge: LlmChallenge;
};
...
export function LlmPlaygroundSolve({
  moduleSlug,
  topicSlug,
  topicTitle,
  moduleTitle,
  challenge,
}: LlmPlaygroundSolveProps) {
  ...
  const topic = useMemo(() => getLlmAcademyTopic(topicSlug), [topicSlug]);
  ...
  const llmRefs = topic?.cheatSheet ?? [];
  ...
  const siblings = useMemo(
    () => listLlmAcademyChallenges(topicSlug),
    [topicSlug]
  );
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-[#0d0d0d]">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 overflow-hidden lg:grid-cols-2">
        <div className="llm-challenge-scroll min-h-0 space-y-5 overflow-y-auto border-r border-zinc-800 px-4 py-5 sm:px-5">
          ...
          {llmRefs.length > 0 ? (
            <div>
              ...
              >
                ...
                {refsOpen ? "Hide LLM reference" : "Show LLM reference"}
              </Button>
              {refsOpen ? (
                <Section title="LLM reference">
