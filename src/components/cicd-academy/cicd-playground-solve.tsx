...
import { MonacoYamlReference } from "@/components/cicd-academy/workspace/monaco-yaml-reference";
import { MonacoShellReference } from "@/components/cicd-academy/workspace/monaco-shell-reference";
...
export function CicdPlaygroundSolve({
  moduleSlug,
  topicSlug,
  topicTitle,
  moduleTitle,
  challenge,
}: CicdPlaygroundSolveProps) {
  ...
  const [codeTab, setCodeTab] = useState<"yaml" | "shell">("yaml");
  ...
  const topic = useMemo(() => getCicdAcademyTopic(topicSlug), [topicSlug]);
  const learnAbout = cleanText(
    challenge.scenario ||
      topic?.summary ||
      "Automated checks and delivery pipelines."
  );

  const yamlCode = challenge.referenceYaml || challenge.starterYaml;
  const shellCode = challenge.referenceShell || challenge.starterShell;
  const cicdRefs = topic?.cheatSheet ?? [];
  ...
  const siblings = useMemo(
    () => listCicdAcademyChallenges(topicSlug),
    [topicSlug]
  );
  ...
  const activeCode = codeTab === "yaml" ? yamlCode : shellCode;
  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-[#0d0d0d]">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 overflow-hidden lg:grid-cols-2">
        <div className="cicd-challenge-scroll min-h-0 space-y-5 overflow-y-auto border-r border-zinc-800 px-4 py-5 sm:px-5">
          ...
          {cicdRefs.length > 0 ? (
            <div>
              ...
              >
                ...
                {refsOpen ? "Hide CI/CD reference" : "Show CI/CD reference"}
              </Button>
              {refsOpen ? (
                <Section title="CI/CD reference">
        ...
        <div className="flex min-h-0 flex-col overflow-hidden bg-[#0a0a0b]">
          ...
          <div className="relative min-h-0 flex-1 overflow-visible">
            {codeTab === "yaml" ? (
              <MonacoYamlReference value={yamlCode} />
            ) : (
