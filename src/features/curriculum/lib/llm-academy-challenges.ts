...
export type LlmChallenge = {
  id: string;
  topicSlug: string;
  title: string;
  difficulty: LearnDifficulty;
  minutes: number;
  kind: LlmChallengeKind;
  scenario: string;
  task: string;
  hints: string[];
  takeaways: string[];
...
function slugToken(topic: LlmTopicDef): string {
...
function defaultPrompt(topic: LlmTopicDef): string {
...
function defaultCode(topic: LlmTopicDef): string {
  return blockJs(topic.title, `export async function runLlmDemo(input) {
  const messages = [
...
function specsForTopic(topic: LlmTopicDef): Spec[] {
  const specs: Spec[] = [];
...
function buildChallenge(topicSlug: string, spec: Spec): LlmChallenge {
  const id = `llm-${topicSlug}-${spec.key}`;
...
const BANK: LlmChallenge[] = flattenLlmTopics().flatMap((topic) =>
  specsForTopic(topic).map((spec) => buildChallenge(topic.slug, spec))
);

const BY_TOPIC = new Map<string, LlmChallenge[]>();
for (const c of BANK) {
...
export function listLlmAcademyChallenges(topicSlug: string): LlmChallenge[] {
  return BY_TOPIC.get(topicSlug) ?? [];
}

export function allLlmAcademyChallenges(): LlmChallenge[] {
  return BANK;
}

export function findLlmAcademyChallenge(
  topicSlug: string,
  challengeId: string
): LlmChallenge | null {
  const list = listLlmAcademyChallenges(topicSlug);
...
export function llmAcademyTopicChallengeCount(topicSlug: string): number {
  return listLlmAcademyChallenges(topicSlug).length;
}

export function isLlmTheoryChallenge(challenge: LlmChallenge): boolean {
